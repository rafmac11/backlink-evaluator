...
const FlagBadge = ({ flag }) => {
  ...
  return (
    <span style={{ display: "inline-block", padding: "2px 10px", borderRadius: 4, border: `1px solid ${color}`, color, fontSize: 11, letterSpacing: 2, fontWeight: 700 }}>
      {flag}
    </span>
  );
};
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 16, padding: "32px" }}>
        ...
        <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 12 }}>Enter the domain you want to explore backlinks for</div>
        ...
        {loading && <p style={{ color: "var(--muted)", fontSize: 12, marginTop: 12, animation: "pulse 2s infinite" }}>Fetching backlink data from DataForSEO...</p>}
      </div>
        ...
        <>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            <input value={filter} onChange={(e) => setFilter(e.target.value)} placeholder="Filter by domain or anchor..."
              style={{ flex: 1, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 8, padding: "8px 14px", color: "var(--text)", fontFamily: "var(--font-mono)", fontSize: 12, outline: "none" }}
              onFocus={(e) => (e.target.style.borderColor = "var(--accent)")}
              onBlur={(e) => (e.target.style.borderColor = "var(--border)")} />
            <span style={{ color: "var(--muted)", fontSize: 12, whiteSpace: "nowrap" }}>{sorted.length} results</span>
          </div>

          <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden", animation: "fadeUp 0.5s ease both" }}>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr>
                    {[
                      { key: "domain_from", label: "LINKING DOMAIN" },
                      { key: "trust_flow", label: "TRUST FLOW" },
                      { key: "domain_from_rank", label: "DOMAIN RANK" },
              ...
              {sorted.length === 0 && (
                <div style={{ padding: 40, textAlign: "center", color: "var(--muted)", fontSize: 13 }}>No backlinks found.</div>
              )}
            </div>
          </div>
        </>
    ...
    <>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "48px 24px" }}>
        <div style={{ marginBottom: 36, animation: "fadeUp 0.4s ease both" }}>
          ...
          <p style={{ color: "var(--muted)", fontSize: 13 }}>AI-powered evaluation · Real backlink data · Trust Flow & Citation Flow scoring</p>
        </div>
